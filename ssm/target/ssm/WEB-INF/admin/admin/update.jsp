...
<head>
  ...
  <title>更新文档 - 光年(Light Year Admin)后台管理系统模板</title>
          ...
          <form action="${base}/admin/update" method="post" enctype="multipart/form-data" class="row">
            ...
            <div class="form-group col-md-12">
              ...
              <div class="form-controls">
                <input type="text" class="form-control" id="name" name="account" value="${a.account}" readonly placeholder="请输入用户名" />
              </div>
            </div>
            <div class="form-group col-md-12">
              ...
              <div class="form-controls">
                <ul class="list-inline clearfix lyear-uploads-pic">
                  <li class="col-xs-4 col-sm-3 col-md-2">
                    <figure>
                      <img id="img" src="${base}/upload/${empty a.headPic?'default.jpg':a.headPic}" alt="图片一">
                      <figcaption>
              ...
              <div class="col-xs-12">
                <select class="form-control" id="example-select" name="roleid" size="1">
                  ...
                  <c:forEach var="r" items="${roles}" >
                    <option value="${r.id}"
                            <c:if test="${a.roleid==r.id}">selected</c:if>
                    >${r.rolename}</option>
                  </c:forEach>
                </select>
              </div>
              ...
              <div class="clearfix">
                <label class="lyear-radio radio-inline radio-primary">
                  ...
                  <input type="radio" name="sex" value="0"
                         <c:if test="${a.sex==0}">checked</c:if>
                  ><span>男</span>
                </label>
                <label class="lyear-radio radio-inline radio-primary">
                  <input type="radio" name="sex" value="1"
                         <c:if test="${a.sex==1}">checked</c:if>
                  ><span>女</span>
                </label>
              </div>
            </div>
              ...
              <div class="clearfix">
                <label class="lyear-radio radio-inline radio-primary">
                  <input type="radio" name="status" value="0"
                    <c:if test="${a.status==0}">checked</c:if>
                  ><span>正常</span>
                </label>
                <label class="lyear-radio radio-inline radio-primary">
                  <input type="radio" name="status" value="1"
                         <c:if test="${a.status==1}">checked</c:if>
                  ><span>冻结</span>
                </label>
                <label class="lyear-radio radio-inline radio-primary">
                  <input type="radio" name="status" value="2"
                         <c:if test="${a.status==2}">checked</c:if>
                  ><span>禁用</span>
                </label>
              </div>
            </div>
